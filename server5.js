...
//http 모듈로부터 server객체 생성하기
var server = http.createServer(function(request, response){
    
    //아래의 코드들이 동작하기 전에, 서버는 클라이언트가 원하는 것이 무엇인지 구분을 먼저 해야한다.
    //요청 분석!!!
    //요청 분석을 들어가려면, 요청 정보를 담고 있는 request 객체를 이용해야 한다!!
    console.log("클라이언트가 요청 시 사용한 url ", request.url);


    //1) 클라이언트가 회원가입 양식을 보기 원하면 아래의 코드가 수행..
    //서버에 존재하는 회원가입 양식 폼파일을 읽어서, 클라이언트 브라우저에 보내주자
    fs.readFile("./회원폼유효성체크.html", "utf-8", function(error, data){
        if(error){
            console.log("에러 ㅜㅜ", error);
        }else{
